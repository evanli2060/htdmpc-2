...
class Ant(PipelineEnv):
  ...
  def __init__(
    ...
    if env_path:
      ...
      path = self.get_resource_filepath()
    sys = mjcf.load(path)
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    forward_reward = velocity[0] * self._forward_reward_weight
    ...
    contact_cost = self._contact_cost_weight * jp.sum(jp.square(contact_forces))
  ...
  def _get_contact_forces(self, pipeline_state: base.State) -> jax.Array:
    raw_contact_forces = pipeline_state.cfrc_ext
    min_value, max_value = self._contact_force_range
    contact_forces = jp.clip(raw_contact_forces, min_value, max_value)
  ...
  def _get_obs(self, pipeline_state: base.State) -> jax.Array:
    ...
    if self._include_cfrc_ext_in_observation:
      contact_force = self._get_contact_forces(pipeline_state)[1:].flatten()
      return jp.concatenate([qpos] + [qvel] + [contact_force])
    else:
      return jp.concatenate([qpos] + [qvel])

  @staticmethod
  def get_resource_filepath() -> str:
    return epath.resource_path('gymnasium') / 'envs/mujoco/assets/ant.xml'
