...
def flip(
  ...
  task = YogaPlanarWalker(
      goal='flip',
      move_speed=walker._RUN_SPEED * 0.75,
      random=random,
  )
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
...
class YogaPlanarWalker(walker.PlanarWalker):
  ...
  def _arabesque_reward(self, physics):
    ...
    upright = (1 - physics.torso_upright()) / 2
    arabesque_reward = (3 * standing + left_foot_down + right_foot_up +
                        upright) / 6
  ...
  def _flip_reward(self, physics):
    ...
    move_reward = rewards.tolerance(
        physics.horizontal_velocity(),
        bounds=(self._move_speed, float('inf')) if self._move_speed > 0 else
        (-float('inf'), self._move_speed),
