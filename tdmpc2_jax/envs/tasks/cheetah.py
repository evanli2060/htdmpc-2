...
def run_backwards(
    time_limit=cheetah._DEFAULT_TIME_LIMIT,
    random=None,
  ...
  task = CustomCheetah(
      goal='run-backwards',
      move_speed=cheetah._RUN_SPEED * 0.8,
      random=random,
  )
...
def run_front(
  ...
  task = CustomCheetah(
      goal='run-front',
      move_speed=cheetah._RUN_SPEED * 0.6,
      random=random,
  )
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
...
class CustomCheetah(cheetah.Cheetah):
  ...
  def _lie_down_reward(self, physics):
    torso_height = physics.named.data.xpos['torso', 'z']
    feet_height = (physics.named.data.xpos['ffoot', 'z'] +
                   physics.named.data.xpos['bfoot', 'z']) / 2
