...
class SequentialReplayBuffer():
  ...
  def insert(self, data: PyTree, env_mask: Optional[np.ndarray] = None) -> None:
    """
    ...
    # Update buffer state
    self.current_inds[env_mask] = (self.current_inds[env_mask] +
                                   1) % self.capacity
  ...
  def sample(self, batch_size: int, sequence_length: int) -> PyTree:
    ...
    start_inds = (
        start_inds -
        (self.sizes[env_inds] - self.current_inds[env_inds])) % self.capacity

    # Sample from buffer and convert from (batch, time, *) to (time, batch, *)
    sequence_inds = start_inds[:, None] + np.arange(sequence_length)
    batch = jax.tree.map(
        lambda x: np.swapaxes(
            x[np.clip(sequence_inds, 0, x.shape[0] - 1), env_inds[:, None]], 0,
            1),
        self.data,
    )
