...
class TDMPC2(struct.PyTreeNode):
  ...
  def update(
    ...
    def world_model_loss_fn(
      ...
      for t in range(self.horizon):
        ...
        zs = zs.at[t + 1].set(z)
        consistency_loss += self.rho**t * jnp.mean(
            (z - next_z[t])**2,
      ...
      _l2 = partial(l2_loss, alpha=1e-4)
      total_loss += _l2(encoder_params) + _l2(dynamics_params) + _l2(
          value_params) + _l2(reward_params) + _l2(
              continue_params) if self.model.predict_continues else 0
    ...
    # Update world model
    (encoder_grads, dynamics_grads, value_grads, reward_grads,
     continue_grads), model_info = jax.grad(
         world_model_loss_fn,
         argnums=(0, 1, 2, 3, 4),
         has_aux=True,
     )(
         ...
         if self.model.predict_continues else None,
     )
    ...
    def policy_loss_fn(params: Dict):
      ...
      policy_loss = ((self.entropy_coef * log_probs - Q).mean(axis=1) *
                     rho).mean()
      policy_loss += l2_loss(params, self.l2_reg)
      return policy_loss, {'policy_loss': policy_loss, 'policy_scale': scale}

    policy_grads, policy_info = jax.grad(
        policy_loss_fn,
        has_aux=True,
    )(self.model.policy_model.params)
  ...
  def estimate_value(
    ...
    for t in range(self.horizon):
      ...
      G += discount * reward.astype(jnp.bfloat16)

      if self.model.predict_continues:
        continues = jax.nn.sigmoid(
            self.model.continue_model.apply_fn(
                {'params': self.model.continue_model.params},
                z,
            )).squeeze(-1) > 0.5
      else:
        continues = 1.0
...
def l2_loss(params, alpha):
  ...
  def leaf_loss(leaf):
    return alpha * jnp.sum(leaf**2)

  return jax.tree.reduce(jnp.add, jax.tree.map(leaf_loss, params))
